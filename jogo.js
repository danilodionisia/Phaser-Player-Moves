...
function preload() {
	...
	game.load.audio('som','assets/som.ogg');
	
	game.load.image('cogumelo', 'assets/cogumelo.png');	
	game.load.audio('tema', 'assets/tema.mp3');		
	game.load.audio('fim', 'assets/fim.mp3');
...
function create() {
	...
	for (var i = 1; i < 9; i++){
        var cogumelo = cogumelos.create(i * 140, 300, 'cogumelo');
    ...
    scoreText = game.add.text(16, 16, 'Placar: 0', { fontSize: '32px', fill: '#F00' });
	
    cursors = game.input.keyboard.createCursorKeys();
	
	level = game.add.text(650, 16, 'Nível 1', { fontSize: '32px', fill: '#FFD851' });
...
function collectStar (player, star, playerName) {
	...
	if(score < 180)
	{
		scoreText.text = playerName + ' - Placar: ' + score;
